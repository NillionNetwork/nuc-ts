...
export function validatePolicyProperties(
  ...
  if (policy.length > config.maxPolicyWidth) {
    Log.debug({ policyLength: policy.length, maxPolicyWidth: config.maxPolicyWidth }, POLICY_TOO_WIDE);
  ...
  if (properties.maxWidth > config.maxPolicyWidth) {
    Log.debug({ maxWidth: properties.maxWidth, maxPolicyWidth: config.maxPolicyWidth }, POLICY_TOO_WIDE);
    throw new Error(POLICY_TOO_WIDE);
  }
  if (properties.maxDepth > config.maxPolicyDepth) {
    Log.debug({ maxDepth: properties.maxDepth, maxPolicyDepth: config.maxPolicyDepth }, POLICY_TOO_DEEP);
